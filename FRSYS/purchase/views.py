...
def purchase_manage_supplierinfo_add_info(request):
    if request.method == 'POST':
        ...
        supplier_charge_phone = request.POST.get('supplier_charge_phone')
        try:
            new_supplier = models.Supplier.add_supplier(supplier_add, supplier_charge_phone, supplier_name,
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            supplier.update_supplier(supplier_id=supplier_id, supplier_name=supplier_name, supplier_add=supplier_add,
                                     supplier_charge_phone=supplier_charge_phone,
...
def purchase_make_purchase_add_purchase(request):
    if request.method == 'POST':
        ...
        try:
            new_purchase_oder = models.Purchase.add_purchase_order(purchase_num=purchase_num,
    ...
    if request.method == 'POST':
        if 'search_purchase_demand' in request.POST:
            ...
            if pdemand_state != '':
                ...
                if pdemand_state == '未完成需求':
                    pdemand_state = 0
                elif pdemand_state == '已完成需求':
                    pdemand_state = 1
...
def order_trace(request):
    if request.method == 'POST':
        ...
        #             deliver_details_order = DeliverDetail.objects.filter(deliver_id_id = deliver_order.deliver_id)
        #             '''  采购过程溯源  '''
        #             # OrderDetail查询集
        ...
        #                 # Outward实例
        #                 outward = order_detail.outward_id
        #                 # Inward实例
        #                 inward = outward.warehouse_flow
        ...
        #                 purchase_detail = PurchaseDetail.objects.get(purchase_id_id=purchase.purchase_id)
        #                 supplier = purchase_detail.supplier_id
        #                 ''' 采购运输过程的溯源 '''
        #                 # Deliver实例
        #                 deliver_purchase = purchase.deliver_id
        #                 # DeliverDetail查询集 这里没法在前端完成，所以，还需要进行讨论，该怎么完成
        #                 deliver_details_purchase = DeliverDetail.objects.filter(deliver_id_id = deliver_purchase.deliver_id)
        #
        #             '''  溯源过程到此结束  '''
        #
        ...
        return render(request, "purchase/findroot/findroot-result.html")  # 搞不清现在啥情况,返回哪个页面也不明白。前端来搞
    elif request.method == 'GET':
        return render(request, "purchase/findroot/findroot-homepage.html")


def fr_result(request):
    return render(request, request, "purchase/findroot/findroot-result.html")
